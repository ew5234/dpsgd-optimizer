...
def theory_var_fun(ep, k, m, y, Cp_assume, index):
    ...
    if (index == 2):
        variance = -Cp ** 2 + 2 * L ** 3 * y / 3 + a ** 2 * k * m / math.pi - 4 * k * m ** 3 / math.pi ** 3 + k * m * (
                    a + m) ** 2 / math.pi
        return float(variance)
    ...
    if (index == 5):
        variance = -Cp ** 2 + 2 * L ** 3 * t / 7 + 8 * L ** 3 * y / 21 + 2 * a ** 2 * k * m / math.pi + 2 * a * k * m ** 2 / math.pii - 4 * k * m ** 3 / math.pi ** 3 + k * m ** 3 / math.pi
        return float(variance)

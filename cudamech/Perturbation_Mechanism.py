import random
from cudamech.Mechanism.Parameter_Optimization import *
import numpy as np
...
def generate_perturbed_list(ep, k, m, y, Cp, index):
    ...
    step = 2 * L / divid
    x_count = -L
    X_axis = np.linspace(-L, L, divid) # Edited: []
    P_axis = np.array([PDF_fun(x_count, ep, k, m, y, Cp, index) for x_count in X_axis]) # Edited: []
    Perturbed_list = []
    ...
    # Changes to original mechanism
    P_axis = P_axis / P_axis.sum()
    Perturbed_list = np.random.choice(X_axis, p=P_axis).tolist()

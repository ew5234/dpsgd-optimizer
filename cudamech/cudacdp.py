...
def perturb_array(A, ep, sensitivity, lower, index, b_size=256, BLOCK_SIZE=32):
    ...
    for b in range(0, len(A), b_size):
        ...
        random_element_rp[total_blocks, BLOCK_SIZE](rp_list_new, cumulated_sum, len(array), rp_list, rp_list_size, random.randint(1,1000))
        ...
        total_blocks = math.ceil(len(rp_list) / BLOCK_SIZE)
        listmapping_inverse_fromLToReal[total_blocks, BLOCK_SIZE](rp_list, sensitivity, lower, ep, k_best, m_best, y_best, index, len(rp_list))
        ...
        results = cupy.concatenate([results, rp_list])
